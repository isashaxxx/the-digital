/* Framer API SDK v0.1.30 */
import ts from "typescript";
import * as CSS from "csstype";

//#region ../plugin-api/src/agent.d.ts
...
interface AgentConversationCompletedResult {
  ...
  /**
   * Snapshot of values stored with `api.store()` in this conversation. Consumers can resolve
   * `data="{{key}}"` template references for custom elements without parsing them in the agent.
   */
...
/** Options for a turn whose prompt is plain text, described by the options themselves. */
interface AgentTextTurnOptions {
  /** How the prompt should be read. Defaults to `text`. */
  promptFormat?: "text";
  /**
...
/**
 * Options for a turn whose prompt is a serialized, migratable user message.
 *
 * The message already describes the whole turn — its content, model, reasoning effort, selection and
 * attachments — so none of the options that would describe it a second time apply.
 */
interface AgentUserMessageTurnOptions {
  /**
   * The prompt is a JSON envelope holding one user message, in the same shape the agent persists a
   * queued prompt in. `version` is the agent schema version the message was written with; messages
   * written by an older client are migrated forward before the turn runs.
   *
   * `parts` is the message content: plain strings interleaved with tags referencing project
   * entities, which is what lets a stored prompt keep pointing at a page after it is renamed.
   *
   * ```json
   * {
   * 	"version": 7,
   * 	"message": {
   * 		"id": "vXaB2p",
   * 		"parts": ["Refresh the testimonials on ", { "type": "page", "pageId": "augiA20Il", "path": "/about", "name": "About" }],
   * 		"model": "anthropic/claude-opus-4.6",
   * 		"reasoningEffort": "low",
   * 		"selection": { "canvas": null, "cms": null },
   * 		"images": [],
   * 		"files": []
   * 	}
   * }
   * ```
   */
  promptFormat: "user-message";
  selectionNodeIds?: never;
  imageUrls?: never;
  fileUrls?: never;
}
type AgentTurnOptions = AgentTextTurnOptions | AgentUserMessageTurnOptions;
/** Correlation metadata supplied by a Slack integration. */
interface AgentSlackMetadata {
  requestId: string;
}
/** Caller-specific metadata when starting an automation conversation. */
type AgentAutomationMetadata = {
  trigger: "manual";
} | {
  trigger: "webhook";
} | {
  trigger: "cron"; /** When this scheduled run fired. */
  triggeredAt: string; /** Cron schedule expression for this automation. */
  expression: string; /** Timezone the cron expression is evaluated in. */
  timezone: string;
} | {
  trigger: "mergeToMain";
  sourceBranchName: string; /** User who merged the branch. */
  userId: string;
} | {
  trigger: "publish"; /** Published site hostname or URL. */
  url: string; /** User who published the site. */
  userId: string;
} | {
  trigger: "deploy"; /** Deployed site hostname or URL. */
  url: string; /** User who started the deploy. */
  userId: string;
};
interface StartAgentConversationSharedOptions {
  /**
...
/**
 * What this conversation is running for.
 *
 * An `automation` run is told that it is non-interactive, that nobody is available to answer a
 * question, and that only its final message is kept. A `slack` run is told to produce a concise,
 * self-contained reply suitable for Slack.
 *
 * Defaults to no particular caller, which leaves the system prompt unchanged.
 */
type StartAgentConversationOptions = AgentTurnOptions & StartAgentConversationSharedOptions & ({
  caller?: undefined;
  metadata?: never;
} | {
  caller: "slack";
  metadata?: AgentSlackMetadata;
} | {
  caller: "automation";
  metadata: AgentAutomationMetadata;
});
type ContinueAgentConversationOptions = AgentTurnOptions & {
  /** Existing agent conversation to continue. */conversationId: string;
} & ({
  caller?: undefined;
  metadata?: never;
} | {
...
//#endregion
//#region ../plugin-api/src/Prettify.d.ts
/**
...
//#endregion
//#region ../plugin-api/src/errors.d.ts
...
//#endregion
//#region ../plugin-api/src/user.d.ts
...
//#endregion
//#region ../plugin-api/src/branches.d.ts
...
type BranchSiteSettingsChangeCategory = "general" | "domains" | "redirects" | "performance" | "forms" | "customCode" | "skills";
...
type BranchChange = BranchCollectionChange | BranchSiteSettingsChange | BranchChangeBase<Exclude<BranchChangeType, "collection" | "siteSettings">>;
...
type AgentBranch = Omit<BranchData, "createdBy">;
interface AgentBranchCollectionItemChange {
  nodeId: string;
  name: string;
  isRemoved: boolean;
}
type AgentBranchChangeSettingsTab = BranchSiteSettingsChangeCategory;
...
//#endregion
//#region ../plugin-api/src/codeFileDiagnostics.d.ts
...
//#endregion
//#region ../plugin-api/src/breakpoint.d.ts
/** @alpha */
...
//#endregion
//#region ../plugin-api/src/internal.d.ts
...
//#endregion
//#region ../plugin-api/src/localization.d.ts
...
interface ListLocalizationGroupsNextRequest {
  cursor?: string;
}
interface ListLocalizationGroupsNextResult {
  group: LocalizationGroup | null;
  cursor?: string;
}
...
//#endregion
//#region ../plugin-api/src/marshal.d.ts
type Marshaled<T> = T extends {
  [$framerInternal.marshal]: () => unknown;
} ? ReturnType<T[typeof $framerInternal.marshal]> : T extends string & {} ? T : T extends number & {} ? T : T extends object ? { [K in keyof T]: Marshaled<T[K]> } : T;
//#endregion
//#region ../plugin-api/src/css.d.ts
...
//#endregion
//#region ../plugin-api/src/utils.d.ts
/**
 ...
 */
type ExplicitPartial<T> = { [P in keyof T]: T[P] | undefined };
...
//#endregion
//#region ../plugin-api/src/fonts.d.ts
...
//#endregion
//#region ../plugin-api/src/text.d.ts
...
//#endregion
//#region ../plugin-api/src/computedValues.d.ts
...
//#endregion
//#region ../plugin-api/src/controls.d.ts
interface WithKey {
  key: string;
}
interface WithTitle {
...
//#endregion
//#region ../plugin-api/src/gradients.d.ts
...
//#endregion
//#region ../plugin-api/src/traits.d.ts
type NodeId = string;
...
//#endregion
//#region ../plugin-api/src/styles.d.ts
...
//#endregion
//#region ../plugin-api/src/variables.d.ts
...
//#endregion
//#region ../plugin-api/src/collectionFields.d.ts
/**
...
//#endregion
//#region ../plugin-api/src/collections.d.ts
...
interface ImageFieldDataEntryInput {
  type: ImageFieldType;
  value: string | null;
  /** The resolution to use for the image. Defaults to "auto". */
...
//#endregion
//#region ../plugin-api/src/viewport.d.ts
...
//#endregion
//#region ../plugin-api/src/nodes.d.ts
interface Rect {
  x: number;
  y: number;
  width: number;
...
interface VectorNodeData extends CommonNodeData {
...
interface PathNodeData extends VectorNodeData {
  [classKey]: "PathNode";
}
interface ShapeContainerNodeData extends VectorNodeData {
 ...
 * @category canvas
 */
declare class ShapeContainerNode extends VectorNode {
 ...
 * @category canvas
 */
declare class PathNode extends VectorNode {
  readonly [classKey]: PathNodeData[ClassKey];
  constructor(rawData: PathNodeData, engine: PluginEngine);
}
interface ShapeGroupNodeData extends VectorNodeData {
 ...
 * @category canvas
 */
declare class BooleanShapeNode extends VectorNode {
...
type CanvasNode = FrameNode | TextNode | ComponentInstanceNode | SVGNode | VectorSetItemNode | ShapeContainerNode | PathNode | ShapeGroupNode | BooleanShapeNode | UnknownNode;
type SomeNodeData = FrameNodeData | TextNodeData | ComponentInstanceNodeData | SVGNodeData | DesignPageNodeData | WebPageNodeData | ComponentNodeData | VectorSetNodeData | VectorSetItemNodeData | ShapeContainerNodeData | PathNodeData | ShapeGroupNodeData | BooleanShapeNodeData | UnknownNodeData;
...
//#endregion
//#region ../plugin-api/src/componentInstancePlaceholder.d.ts
/** @alpha */
...
//#endregion
//#region ../plugin-api/src/codeFile.d.ts
...
//#endregion
//#region ../plugin-api/src/customCode.d.ts
...
//#endregion
//#region ../plugin-api/src/serverApiOnly.d.ts
...
//#endregion
//#region ../plugin-api/src/vectors.d.ts
type Ownership = {
  type: "project";
} | {
  type: "external";
...
type Method = keyof { [K in keyof AllMembers as AllMembers[K] extends ((...args: any) => unknown) ? K : never]: string };
declare const methodToMessageTypes: {
  readonly addComponentInstance: ["addComponentInstance"]; /** @alpha */
  ...
  readonly "Branch.getChanges": [];
  readonly "Branch.switch": ["switchBranch"];
  readonly "Branch.join": ["joinBranch"];
  readonly "Branch.leave": ["leaveBranch"];
  readonly "Branch.rename": ["renameBranch"];
  readonly "Branch.delete": ["deleteBranch"];
  readonly "Branch.merge": ["mergeBranch"];
  ...
  readonly listLocalizationGroups: [];
  ...
  readonly setCloseWarning: []; /** @deprecated The lintCode API was removed. */
  readonly lintCode: [];
  ...
  readonly subscribeToDeployment: [];
  readonly subscribeToBranches: [];
  readonly subscribeToActiveBranch: [];
  ...
  readonly unstable_getDependencyVersion: [];
  ...
  readonly "CodeFile.getVersions": []; /** @alpha */
  readonly "CodeFile.showProgressOnInstances": []; /** @alpha */
  readonly "CodeFile.removeProgressFromInstances": [];
  readonly "CodeFile.lint": [];
  readonly "CodeFile.typecheck": [];
  readonly "CodeFileVersion.getContent": []; /** @alpha */
  readonly "ComponentInstancePlaceholder.setAttributes": []; /** @alpha */
  readonly "ComponentInstancePlaceholder.remove": []; /** @alpha */
  ...
  readonly "TextNode.getHTML": []; /** @alpha */
  readonly "ComponentNode.addVariant": ["addVariantToComponent"]; /** @alpha */
  readonly "ComponentNode.addGestureVariant": ["addGestureVariantToComponent"]; /** @alpha */
  readonly "ComponentNode.getVariables": []; /** @alpha */
  readonly "ComponentNode.addVariables": ["addVariables"]; /** @alpha */
  readonly "ComponentNode.removeVariables": ["removeVariables"];
  readonly "WebPageNode.getBreakpointSuggestions": [];
  readonly "WebPageNode.addBreakpoint": ["addBreakpointToWebPage"]; /** @alpha */
  ...
  readonly "TextStyle.setPluginData": ["setPluginDataForNode"]; /** @alpha */
  readonly "Variable.setAttributes": ["updateVariable"]; /** @alpha */
  readonly "Variable.remove": ["removeVariables"]; /** @alpha */
  readonly "ComponentNode.setVariableOrder": ["setVariableOrder"];
  readonly "EnumCase.remove": ["removeEnumCase"];
  readonly "EnumCase.setAttributes": ["updateEnumCase"]; /** @alpha */
  readonly "EnumVariable.addCase": ["addEnumCase"]; /** @alpha */
  ...
  readonly getDeploymentIssues: [];
  readonly getUnpublishedPageChanges: [];
...
type AllMethods = keyof { [K in Method as (typeof methodToMessageTypes)[K] extends [] ? never : K]: (typeof methodToMessageTypes)[K] };
type ProtectedMethod = AllMethods & string;
type PerMethodPermissionMap = { [K in ProtectedMethod]: boolean };
//#endregion
//#region ../plugin-api/src/publishInfo.d.ts
...
type DeploymentStatus = "pending" | "optimizing" | "ready" | "failed";
type DeploymentFailureStage = "publish" | "optimization";
type DeploymentIssueSeverity = "warning" | "error";
...
type Deployment = (DeploymentBase & {
...
interface ListDeploymentsPage {
  deployments: Deployment[];
  cursor?: string;
  hasNextPage: boolean;
}
type HostnameType = "default" | "custom" | "version" | "branch";
interface Hostname {
  hostname: string;
  type: HostnameType;
  isPrimary: boolean;
...
//#endregion
//#region ../plugin-api/src/redirects.d.ts
...
//#endregion
//#region ../plugin-api/src/theme.d.ts
...
//#endregion
//#region ../plugin-api/src/messages.d.ts
...
type PluginSubscriptionEvent = PluginSubscriptionPublishInfo | PluginSubscriptionDeployment | PluginSubscriptionSelection | PluginSubscriptionCanvasRoot | PluginSubscriptionImage | PluginSubscriptionTheme | PluginSubscriptionText | PluginSubscriptionCustomHTML | PluginSubscriptionColorStyle | PluginSubscriptionTextStyle | /** @alpha */PluginSubscriptionRedirects | PluginSubscriptionCodeFiles | /** @alpha */PluginSubscriptionOpenCodeFile | PluginSubscriptionBranches | PluginSubscriptionActiveBranch;
...
//#endregion
//#region ../plugin-api/src/initialState.d.ts
...
//#endregion
//#region ../plugin-api/src/menu.d.ts
...
//#endregion
//#region ../plugin-api/src/notifications.d.ts
...
interface NotifyOptions extends NotifyOptionsBase {
  ...
  button?: {
    /** The text of the button */text: string; /** Click handler when the button is pressed */
    onClick: () => void;
  };
...
//#endregion
//#region ../plugin-api/src/engine.d.ts
...
declare class PluginEngine {
  ...
  readonly mode: Mode;
  readonly subscriptions: Map<"image" | "publishInfo" | "deployment" | "selection" | "canvasRoot" | "theme" | "text" | "customCode" | "colorStyles" | "textStyles" | "redirects" | "codeFiles" | "openCodeFile" | "branches" | "activeBranch", Set<(value: any) => void>>;
...
//#endregion
//#region ../plugin-api/src/assets.d.ts
type AssetId = string;
...
interface AssetData extends WithOptionalName$1 {
...
interface NamedFileAssetInput extends WithOptionalName$1 {
  file: AssetInput;
}
...
interface ImageData extends WithOptionalName$1, ImageOptions {}
type AssetDataTransfer = AssetURLDataTransfer | BytesDataTransfer;
type NamedImageTransfer = AssetDataTransfer & ImageData;
type NamedAssetTransfer = AssetDataTransfer & WithOptionalName$1;
interface SVGData extends WithOptionalName$1 {
  svg: string;
}
//#endregion
//#region ../plugin-api/src/dragging.d.ts
...
//#endregion
//#region ../plugin-api/src/unpublishedPageChanges.d.ts
type UnpublishedPageChangeStatus = "added" | "updated" | "removed";
interface UnpublishedPageChangeData {
  nodeId: NodeId;
  path: string;
  status: UnpublishedPageChangeStatus;
}
/**
 * A publish-relative change to a web page.
 * @category branching
 */
declare class UnpublishedPageChange {
  #private;
  constructor(data: UnpublishedPageChangeData, engine: PluginEngine);
  /** The stable node identifier for this change. */
  get nodeId(): NodeId;
  /**
   * The page path. This is the current path for added and updated pages, and the
   * last-published path for removed pages.
   */
  get path(): string;
  /** The operation required to publish this change. */
  get status(): UnpublishedPageChangeStatus;
  /**
   * Looks up the current web page node. Each call performs a fresh lookup.
   * Removed pages and nodes that no longer resolve to a web page return `null`.
   */
  getNode(): Promise<WebPageNode | null>;
}
//#endregion
//#region ../plugin-api/src/framerAPI.d.ts
/**
...
declare class FramerPluginAPI {
  ...
  protected get engine(): PluginEngine;
  /**
   ...
   * ```
   *
   * For large exports, prefer {@link FramerPluginAPIBeta.listLocalizationGroups} to avoid
   * accumulating the complete result in memory.
   * @category localization
   */
...
declare class FramerPluginAPIBeta extends FramerPluginAPI {
   ...
   *
   * This pull-based API requests the next group only when the consumer asks for it, avoiding
   * accumulation of the complete result in memory.
   *
   * @example
   * ```ts
   ...
   * @category localization
   */
  listLocalizationGroups(): AsyncIterable<LocalizationGroup>;
}
/** @alpha */
...
interface AgentComponentCatalogEntry {
  id: string;
  displayName: string;
}
interface AgentCodeComponentCatalogEntry extends AgentComponentCatalogEntry {
  type: "component" | "override" | "shader";
}
interface AgentAdditionalComponentCatalogEntry extends AgentComponentCatalogEntry {
  keywords: string | null;
}
interface AgentComponentCatalog {
  project: {
...
declare class FramerAgentAPI {
  ...
  /**
   * Lists available icon set names grouped by current project, external, and additional insertable sets.
   *
   * @returns Current project, external, and additional insertable icon set names.
   */
  listIconSets(): Promise<AgentIconSetCatalog>;
  /**
 ...
 */
type FramerApiOnlyMethods = { [K in Exclude<keyof typeof $framerApiOnly, "getChangedPaths" | "getDeployments">]: FramerPluginAPIAlpha[(typeof $framerApiOnly)[K]] } & {
  /** @deprecated Use `getUnpublishedPageChanges` instead. */getChangedPaths: FramerPluginAPIAlpha[(typeof $framerApiOnly)["getChangedPaths"]]; /** @deprecated Use `listDeployments` instead. */
...
interface PluginMessageAPI {
  ...
  listLocalizationGroupsNext: (request: ListLocalizationGroupsNextRequest) => Promise<ListLocalizationGroupsNextResult>;
  releaseLocalizationGroupsCursor: (cursor: string) => Promise<void>;
  ...
  /** @alpha */
  listIconSetsForAgent: (options?: {
    pagePath?: string;
  }) => Promise<AgentIconSetCatalog>;
  /** @alpha */
...
//#endregion
//#region src/public/reexport.d.ts
interface FramerPlugin extends FramerPluginAPIAlpha {}
declare const framer: FramerPlugin;
//#endregion
//#region src/util/env.d.ts
declare function configure(environment: Record<string, string | undefined>): void;
//#endregion
//#region src/util/errors.d.ts
declare enum ErrorCode {
  PROJECT_CLOSED = "PROJECT_CLOSED",
  EXPECTED_BRANCH_MISSING = "EXPECTED_BRANCH_MISSING",
  POOL_EXHAUSTED = "POOL_EXHAUSTED",
  TOKEN_SESSION_LIMIT = "TOKEN_SESSION_LIMIT",
  TIMEOUT = "TIMEOUT",
  INTERNAL = "INTERNAL",
  NODE_NOT_FOUND = "NODE_NOT_FOUND",
  SCREENSHOT_TOO_LARGE = "SCREENSHOT_TOO_LARGE",
  SCREENSHOT_PREPARE_FAILED = "SCREENSHOT_PREPARE_FAILED",
  EXPORT_SVG_FAILED = "EXPORT_SVG_FAILED",
  INVALID_REQUEST = "INVALID_REQUEST",
  INVALID_RELEASE_CHANNEL = "INVALID_RELEASE_CHANNEL",
  UNAUTHORIZED = "UNAUTHORIZED"
}
interface FramerAPIErrorOptions {
  /** Short server-issued correlation tag (e.g. crash incident id). Surfaced
   *  on the error instance as `err.ref` and embedded into console output by
   *  the WS transport. It must NOT be folded into telemetry payloads
...
declare const enabledMethods: {
  ...
  subscribeToDeployment: false;
  subscribeToBranches: false;
  subscribeToActiveBranch: false;
  unstable_getDependencyVersion: false;
  ...
  getCanvasRoot: true;
  getPublishInfo: true;
  getUnpublishedPageChanges: true;
  publish: true;
  getDeployment: true;
  getDeploymentIssues: true;
  ...
  createCodeFile: true;
  getCodeFiles: true;
  getCodeFile: true; /** @deprecated The lintCode API was removed. */
  lintCode: true;
  typecheckCode: true;
  addRedirects: true;
  getRedirects: true;
  ...
  listLocalizationGroups: true;
};
type EnabledMethodsConfig = typeof enabledMethods;
type BlockedMethods = { [K in keyof EnabledMethodsConfig]: EnabledMethodsConfig[K] extends false ? K : never }[keyof EnabledMethodsConfig];
...
//#endregion
export { $framerApiInternal, type AgentClarificationAnswer, type AgentClarificationQuestion, type AgentClarificationSuggestedAnswer, type AgentConversationCompletedResult, type AgentConversationNeedsClarificationResult, type AgentResponseContentPart, type AgentResponseMessage, AllTraits, type AnyNode, type ApiVersion1ProjectInfo, type ApiVersion1User, type ArrayControl, type ArrayFieldDataEntry, type ArrayFieldDataEntryInput, type ArrayItem, type ArrayItemData, type ArrayItemInput, AxisOverflow, type BooleanControl, BooleanField, BooleanVariable, Border, type BorderControl, BorderRadius, type BorderRadiusControl, BorderStyle, BorderVariable, BorderWidth, type Breakpoint, type CanvasNode, type CanvasRootNode, type CodeFile, type CodeFileComponentExport, type CodeFileExport, type CodeFileOverrideExport, type CodeFileVersion, type Collection, type CollectionItem, type CollectionItemData, type CollectionItemInput, type CollectionReferenceControl, CollectionReferenceField, type ColorControl, ColorField, type ColorStop, type ColorStyle, ColorVariable, ComponentInstanceNode, type ComponentInstancePlaceholder, type ComponentInstancePlaceholderAttributes, type ComponentInstancePlaceholderData, ComponentNode, type ComponentVariable, type ComputedValue, ConicGradient, type ConnectOptions, type ContinueAgentConversationResult, type Control, ControlAttributes, type CreateField, type CreateVariable, type CursorControl, type CustomCode, type CustomCodeLocation, type CustomCursorControl, type DateControl, DateField, DateVariable, type Deployment, type DeploymentFailureStage, type DeploymentIssue, type DeploymentIssueSeverity, type DeploymentStatus, DesignPageNode, type DiagnosticSpan, type EditableManagedCollectionField, EnumCase, type EnumCaseData, type EnumControl, EnumField, EnumVariable, ErrorCode, type Field, type FieldData, type FieldDataEntry, type FieldDataEntryInput, type FieldDataInput, FieldDivider, type FileControl, FileField, FileVariable, FitContent, FitImage, type Font, type FontControl, type FormattedTextControl, FormattedTextField, FormattedTextVariable, FrameNode, type Framer, FramerAPIError, FramerPlugin, FramerPluginClosedError, FramerPluginError, type FusedNumberControl, type GapControl, Gesture, type Gradient, GridContentAlignment, GridItemAlignment, GridItemColumnSpan, GridLayout, HeightConstraint, HeightLength, type Hostname, type HostnameType, type ImageAsset, type ImageControl, ImageField, ImageRendering, ImageVariable, type InlineLocalizationValueByLocale, IsBreakpoint, IsComponentGestureVariant, IsComponentVariant, LayoutType, Length, LinearGradient, type LinkControl, LinkField, type LinkRelControl, LinkVariable, type LintConfig, type LintDiagnostic, type LintLink, type Locale, type LocaleId, type LocalizationData, type LocalizationGroup, type LocalizationGroupStatus, type LocalizationGroupStatusByLocale, type LocalizationSource, type LocalizationSourceId, type LocalizationSourceUpdate, type LocalizationValueByLocale, type LocalizedValueStatus, type LocalizedValueUpdate, type ManagedCollection, type ManagedCollectionField, type ManagedCollectionFieldInput, type ManagedCollectionItemInput, type Mode, type MultiCollectionReferenceControl, MultiCollectionReferenceField, NodeAttributeKey, NodeId, type NodeRuntimeErrorResult, type NumberControl, NumberField, NumberVariable, type ObjectControl, Overflow, type Ownership, type PaddingControl, type PageScopeControl, Position, type ProjectInfo, type ProtectedMethod, type Publish, type PublishInfo, type PublishResult, RadialGradient, type Rect, type Redirect, type RedirectInput, SVGNode, type ScreenshotOptions, type ScreenshotResult, type ScrollSectionControl, type SetLocalizationDataResult, type ShadowControl, type ShowProgressOnInstancesAttributes, StackAlignment, StackDirection, StackDistribution, StackLayout, type StartAgentConversationResult, type StringControl, StringField, StringVariable, type SubmitAgentClarificationResult, SupportedLinkRelValue, type TextAlignment, type TextDecoration, TextNode, type TextStyle, type TextStyleBreakpoint, type TextStyleTag, type TextTransform, type TrackingIdControl, TraitVariant, TraitVariantData, TraitVariantNode, type TransitionControl, type TypecheckDiagnostic, UnpublishedPageChange, type UnpublishedPageChangeStatus, UnsupportedComputedValue, UnsupportedField, UnsupportedVariable, type UpdateFieldAttributes, type User, type Variable, VectorSet, type VectorSetData, VectorSetItem, type VectorSetItemControl, type VectorSetItemData, VectorSetItemNode, type VectorSetItemVariable, VectorSetNode, WebPageNode, WidthConstraint, WidthLength, WithAspectRatioTrait, WithBackgroundColorTrait, WithBackgroundGradientTrait, WithBackgroundImageTrait, WithBorderRadiusTrait, WithBorderTrait, WithBreakpointTrait, WithComponentInfoTrait, WithComponentVariantTrait, WithControlAttributesTrait, WithFontTrait, WithGridItemTrait, WithIdTrait, WithImageRenderingTrait, WithInlineTextStyleTrait, WithLayoutTrait, WithLinkTrait, WithLockedTrait, WithNameTrait, WithNullableComponentInfoTrait, WithOpacityTrait, WithOverflowTrait, WithPinsTrait, WithPositionTrait, WithReplicaInfoTrait, WithRequiredComponentInfoTrait, WithRotationTrait, WithSVGTrait, WithSizeConstraintsTrait, WithSizeTrait, WithTextTruncationTrait, WithVisibleTrait, WithWebPageInfoTrait, WithZIndexTrait, configure, connect, framer, hasGridLayout, hasStackLayout, isBreakpoint, isCodeFileComponentExport, isCodeFileOverrideExport, isColorStyle, isComponentGestureVariant, isComponentInstanceNode, isComponentNode, isComponentVariable, isComponentVariant, isComputedValue, isDesignPageNode, isField, isFileAsset, isFrameNode, isImageAsset, isRetryableError, isSVGNode, isTextNode, isTextStyle, isVariable, isVectorSetItemNode, isVectorSetNode, isWebPageNode, supportsAspectRatio, supportsBackgroundColor, supportsBackgroundColorData, supportsBackgroundGradient, supportsBackgroundGradientData, supportsBackgroundImage, supportsBackgroundImageData, supportsBorder, supportsBorderRadius, supportsBreakpoint, supportsComponentInfo, supportsComponentVariant, supportsFont, supportsFontData, supportsImageRendering, supportsInlineTextStyle, supportsInlineTextStyleData, supportsLayout, supportsLink, supportsLocked, supportsName, supportsOpacity, supportsOverflow, supportsPins, supportsPosition, supportsRotation, supportsSVG, supportsSize, supportsSizeConstraints, supportsTextTruncation, supportsVisible, supportsZIndex, withConnection };