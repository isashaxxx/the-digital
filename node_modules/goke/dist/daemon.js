...
function writePidFile(filePath, data) {
 ...
 */
function removePidFileIfOwned(filePath, instanceId) {
    try {
...
class DaemonContext {
    ...
    #timeoutTimer = null;
    constructor({ cliName, commandName, argv, env, stdout = process.stdout, stderr = process.stderr, }) {
    ...
    publishStartupMessage(message, options) {
        if (!this.isDaemon) {
    ...
    async #waitForStartup({ child, filePath, token, timeoutMs, }) {
 ...
 */
function createDaemonContext(options) {
