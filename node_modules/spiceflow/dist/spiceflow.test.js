...
import { bfs, cloneDeep, createSafePath, Spiceflow } from "./spiceflow.js";
import { z } from 'zod';
...
describe('safePath', () => {
    ...
    test('safePath appends query params', () => {
        ...
        expect(app.safePath('/search', { q: 'hello', page: 1 })).toBe('/search?q=hello&page=1');
        expect(app.safePath('/users/:id', { id: '42', fields: 'name' })).toBe('/users/42?fields=name');
    ...
    test('safePath with query params and no path params', () => {
        ...
        expect(app.safePath('/items', { sort: 'date', limit: 10 })).toBe('/items?sort=date&limit=10');
    ...
    test('safePath with .route and query', () => {
        ...
        expect(app.safePath('/api/search', { term: 'test' })).toBe('/api/search?term=test');
    ...
    test('safePath skips undefined/null query values', () => {
        ...
        expect(app.safePath('/filter', { a: 'yes', b: undefined })).toBe('/filter?a=yes');
    });
    test('safePath query with basePath', () => {
        const app = new Spiceflow({ basePath: '/api' }).get('/search', () => 'search', {
            query: z.object({ q: z.string() }),
        });
        expect(app.safePath('/api/search', { q: 'hello' })).toBe('/api/search?q=hello');
    ...
    test('safePath query with all HTTP method shorthands', () => {
        ...
        expect(app.safePath('/put-q', { x: 'val' })).toBe('/put-q?x=val');
        expect(app.safePath('/patch-q', { y: 5 })).toBe('/patch-q?y=5');
        expect(app.safePath('/del-q', { confirm: true })).toBe('/del-q?confirm=true');
    ...
    test('safePath routes without query schema accept arbitrary query at runtime', () => {
        ...
        expect(app.safePath('/no-schema', { anything: 'works' })).toBe('/no-schema?anything=works');
    });
});
describe('createSafePath', () => {
    ...
    test('works with path params', () => {
        ...
        expect(safePath('/posts/:postId/comments/:commentId', {
            postId: 'abc',
            commentId: '456',
        })).toBe('/posts/abc/comments/456');
    ...
    test('works with query params and rejects invalid keys', () => {
        ...
        const safePath = createSafePath(app);
        expect(safePath('/search', { q: 'hello', page: 1 })).toBe('/search?q=hello&page=1');
    ...
    test('works with both path and query params', () => {
        ...
        const safePath = createSafePath(app);
        expect(safePath('/users/:id', { id: '42', fields: 'name' })).toBe('/users/42?fields=name');
    ...
    test('rejects invalid query keys across multiple routes', () => {
        ...
        const safePath = createSafePath(app);
        expect(safePath('/items', { sort: 'name', limit: 10 })).toBe('/items?sort=name&limit=10');
        expect(safePath('/create', { dryRun: true })).toBe('/create?dryRun=true');
    ...
    test('works with .route and rejects invalid query keys', () => {
        ...
        const safePath = createSafePath(app);
        expect(safePath('/api/data', { format: 'json' })).toBe('/api/data?format=json');
    ...
    test('works with basePath and rejects invalid query keys', () => {
        const app = new Spiceflow({ basePath: '/v2' }).get('/users', () => 'users', {
            query: z.object({ active: z.boolean() }),
        });
        const safePath = createSafePath(app);
        expect(safePath('/v2/users', { active: true })).toBe('/v2/users?active=true');
        // @ts-expect-error - invalid query key
    ...
    test('without query schema allows arbitrary query', () => {
        ...
        const safePath = createSafePath(app);
        expect(safePath('/free', { any: 'value', works: 'here' })).toBe('/free?any=value&works=here');
    });
    test('partial query params are accepted', () => {
        ...
        const safePath = createSafePath(app);
        expect(safePath('/filter', { a: 'only-a' })).toBe('/filter?a=only-a');
        expect(safePath('/filter', { a: '1', c: '3' })).toBe('/filter?a=1&c=3');
    });
    test('mixed routes with and without query schemas', () => {
        ...
        expect(safePath('/untyped', { anything: 'goes' })).toBe('/untyped?anything=goes');
        expect(safePath('/also-typed/:id', { id: '1', verbose: true })).toBe('/also-typed/1?verbose=true');
