...
() => {
    ...
    .get('/search', ({ query }) => query, { query: z.object({ q: z.string(), page: z.coerce.number().optional() }) });
...
describe('fetch client', () => {
    ...
    it('handles thrown response with 200', async () => {
        ...
        if (result instanceof Error)
            throw result;
        expect(result).toMatchInlineSnapshot(`"this string will not be parsed as json"`);
    });
    ...
    it('untyped URL falls back gracefully', async () => {
        ...
        const result = await untypedFetch('/number');
        if (result instanceof Error)
            throw result;
        expect(result).toEqual(1);
    });
